...
describe("Test of Photo Jigsaw Puzzle Widget", function() {
  ...
  return describe("Puzzle Grid Model Tests", function() {
    ...
    return it("should convert hex coordinates to pixel coordinates", function() {
      expect(this.grid.get_xy(1, 1)).toEqual([115, 55]);
      expect(this.grid.get_xy(1, 2)).toEqual([115, 74]);
      expect(this.grid.get_xy(1, 3)).toEqual([115, 93]);
      expect(this.grid.get_xy(2, 1)).toEqual([129, 45]);
      expect(this.grid.get_xy(2, 2)).toEqual([129, 64]);
      expect(this.grid.get_xy(2, 3)).toEqual([129, 83]);
      expect(this.grid.get_xy(3, 1)).toEqual([144, 55]);
      expect(this.grid.get_xy(3, 2)).toEqual([144, 74]);
      expect(this.grid.get_xy(3, 3)).toEqual([144, 93]);
      expect(this.grid.get_xy(4, 1)).toEqual([158, 45]);
      expect(this.grid.get_xy(4, 2)).toEqual([158, 64]);
      return expect(this.grid.get_xy(4, 3)).toEqual([158, 83]);
    });
  });
});
